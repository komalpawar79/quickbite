...
const NotificationsModule = () => {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      ...
      // Access from res.data since ApiResponse wraps the data
      const notificationsData = res.data?.notifications || res.notifications || [];
      const pagination = res.data?.pagination || res.pagination || {};
      
      setNotifications(notificationsData);
      setUnreadCount(pagination.unread || 0);
    } catch (error) {
  ...
  const handleMarkAllAsRead = async () => {
    ...
    try {
      ...
      setUnreadCount(0);
      toast.success(`✓ All ${response.data?.marked || 'all'} notifications marked as read!`);
    } catch (error) {
  ...
  return (
    <div className="space-y-6 p-2">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-green-100 text-sm mt-1">Stay updated with all system events</p>
            </div>
          </div>
          <div className="text-right">
            <div className="text-4xl font-bold">{filteredNotifications.length}</div>
            <p className="text-green-100 text-xs mt-1">Items</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between bg-white p-4 rounded-xl shadow-sm border border-gray-200">
        ...
        {unreadCount > 0 && (
          <button
            ...
            disabled={markingAll}
            className="px-6 py-2.5 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white rounded-lg hover:shadow-lg transition-all duration-300 font-medium hover:scale-105 active:scale-95 disabled:opacity-75 disabled:cursor-not-allowed flex items-center gap-2"
          >
            <span className={markingAll ? 'animate-spin' : ''}>{markingAll ? '⟳' : '✓'}</span>
                ...
                <div className="flex gap-2 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  {!notification.isRead && (
                    <button
                      ...
                      className="px-4 py-2.5 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white rounded-lg text-xs font-semibold transition-all hover:scale-110 hover:shadow-lg active:scale-95 shadow-md flex items-center gap-1.5"
                      title="Mark as read"
                    >
                  ...
                  <button
                    ...
                    className="px-4 py-2.5 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white rounded-lg text-xs font-semibold transition-all hover:scale-110 hover:shadow-lg active:scale-95 shadow-md flex items-center gap-1.5 disabled:opacity-75"
                    disabled={deleting === notification._id}
                    title="Delete"
                  >
                    <span className={`text-sm ${deleting === notification._id ? 'delete-spinner' : ''}`}>{deleting === notification._id ? '⟳' : '✕'}</span>
                    <span>{deleting === notification._id ? 'Deleting...' : 'Delete'}</span>
                  </button>
                </div>
              </div>
            </div>
      ...
      <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center p-3 bg-gradient-to-br from-green-50 to-green-100 rounded-lg border border-green-200">
            ...
            <p className="text-xs text-green-600 font-medium mt-1">Total</p>
          </div>
      ...
      <style>{`
        ...
        /* Animation for deleting spinner */
        @keyframes spin {
